...
type Event = { _id: string; date: string; attendees?: number; eventType?: string }
...
export function ImpactDashboard() {
  ...
  const reached = useMemo(() => events.reduce((total, event) => total + (Number(event.attendees) || 0), 0), [events])
  const completed = projects.filter(project => project.status === "Completed").length
  const yearly = useMemo(() => {
    const counts = new Map<number, { events: number; projects: number; reached: number }>()
    events.forEach(event => { const year = new Date(event.date).getFullYear(); if (!Number.isFinite(year)) return; const row = counts.get(year) || { events: 0, projects: 0, reached: 0 }; row.events++; row.reached += Number(event.attendees) || 0; counts.set(year, row) })
    projects.forEach(project => { const year = new Date(project.date).getFullYear(); if (!Number.isFinite(year)) return; const row = counts.get(year) || { events: 0, projects: 0, reached: 0 }; row.projects++; counts.set(year, row) })
    return [...counts.entries()].sort(([a],[b]) => a-b)
  }, [events, projects])
  const maxActivity = Math.max(1, ...yearly.map(([, row]) => row.events + row.projects))

  const metrics = [
    [CalendarDays, events.length, "Published events"], [FolderKanban, projects.length, "Projects"], [Users, reached, "People engaged"],
